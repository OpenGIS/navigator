...
Navigator includes built-in multi-language support. The active language is detected automatically from the browser, with an English fallback. Library consumers can supply a default language and override any label via `Navigator.create()`. Users can also change their language in the Settings panel.
...
2. **Config default** — the `locale` option passed to `Navigator.create()`
...
> [!NOTE]
> **RTL language support** is planned for a future release. The navigation bar already uses directional positioning (`'start'` / `'end'` rather than left / right) and Bootstrap 5 logical utilities, so the layout will adapt automatically when RTL support lands. See [Extending — Custom Buttons](../extend/2.buttons-panels.md#custom-buttons) for the toolbar positioning API.
...
| `modal.welcome.*` | `modal.welcome.title` | About Navigator |
| `settings.*` | `settings.darkMode` | Dark mode |
| `locate.*` | `locate.button.locate` | Locate |
| `modal.locate.*` | `modal.locate.permissionTitle` | Permission Required |

> [!TIP]
> For the complete list of translation keys, see the source locale files:
> - [`src/locales/en.json`](../../src/locales/en.json) — English (canonical)
> - [`src/locales/fr.json`](../../src/locales/fr.json) — Français
