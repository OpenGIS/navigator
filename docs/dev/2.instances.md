...
`Navigator.create()` returns a `NavigatorInstance` — a wrapper around the Vue app that provides event emitter methods and mount/unmount lifecycle control. See [Extending](./8.extending.md) for full details.
